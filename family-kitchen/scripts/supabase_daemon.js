...
async function log(msg) {
  ...
  if (currentJobId) {
    try {
      await supabase.from('checkout_jobs').update({ logs: currentLogs }).eq('id', currentJobId);
    } catch (e) {
...
async function processJob(job) {
  ...
  try {
    ...
    try {
      ...
      log(`NOTICE: System Chrome unavailable (${launchErr.message?.split('\n')[0]}). Trying Playwright Chromium...`);
...
async function watchQueue() {
  console.log('Listening for checkout jobs on Supabase (checkout_jobs table)...');

  supabase.channel('custom-insert-channel')
    .on(
      'postgres_changes',
      { event: 'INSERT', schema: 'public', table: 'checkout_jobs', filter: "status=eq.pending" },
      async (payload) => {
        const jobRow = payload.new;
        console.log(`New job received: ${jobRow.id}`);
        ...
        if (error || !data) {
          console.log(`Job ${jobRow.id} already claimed or errored.`);
          return;
        }
        ...
        try {
          await processJob(jobRow.payload);
          await supabase.from('checkout_jobs').update({ status: 'completed' }).eq('id', currentJobId);
        } catch (err) {
          await log(`ERROR: ${err.message}`);
          await supabase.from('checkout_jobs').update({ status: 'error' }).eq('id', currentJobId);
        } finally {
          currentJobId = null;
          activeBrowser = null;
        }
    ...
    .on(
      'postgres_changes',
      { event: 'UPDATE', schema: 'public', table: 'checkout_jobs' },
      async (payload) => {
        const jobRow = payload.new;
        if (jobRow.id === currentJobId && jobRow.status === 'cancelled') {
          console.log(`Job ${currentJobId} was cancelled by user.`);
          if (activeBrowser) {
